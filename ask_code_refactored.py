#!/usr/bin/env python3
"""
Rails Code Analysis CLI using Refactored ReAct Agent

Enhanced version of ask_code.py that uses the new refactored agent architecture
...
# Import refactored agent and compatibility layer
from agent.refactored_rails_agent import RefactoredRailsAgent
from agent.config import AgentConfig
from agent.compatibility import create_agent_for_ask_code
...
def get_agent_input(console, prompt_style, display_string, thinking_mode, user_history, tools_enabled):
    """
    Enhanced input function for the refactored Rails agent.
    ...
    def _display_enhanced_instructions(token_info: str = None, thinking_mode: bool = False) -> None:
        """Display enhanced usage instructions for the refactored agent."""
    ...
    try:
        ...
        if user_input.strip().lower() == "/tools":
            console.print("[dim]Tools are always enabled for Rails analysis (refactored agent)[/dim]")
            return None, False, thinking_mode, tools_enabled
...
def repl(
    ...
    *,
    ...
    """
    Enhanced interactive Rails code analysis loop with refactored ReAct agent.
    ...
    """
    console.rule("🚀 Enhanced Rails Analysis Agent (Refactored)")
    ...
    # Initialize refactored ReAct agent
    try:
        react_agent = create_agent_for_ask_code(project_root, session, debug)
        ...
        console.print(f"[red]Error: Could not initialize refactored ReAct agent: {e}[/red]")
        if debug:
            import traceback
    ...
    # Track UI state
    thinking_mode = False
    tools_enabled = True  # Always enabled for refactored agent
    user_history = []
    ...
    while True:
        ...
        # Process through refactored ReAct agent
        try:
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="ask-code-refactored",
        description="Enhanced Rails Code Analysis Agent (Refactored Architecture)",
